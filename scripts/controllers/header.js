...
SwaggerEditor.controller('HeaderCtrl', function HeaderCtrl($scope, $uibModal,
  ...
  $scope.pdfGet = function () {
    console.log("working");
    var content = $rootScope.editorValue;
    $http({
      method: 'POST',
      url: '/pdf',
  ...
  function assignDownloadHrefs() {
    ...
    YAML.load(yaml, function(error, json) {
      ...
      var assign = function(yaml, json) {
        ...
        $scope.jsonDownload = 'data:application/json;charset=utf-8,' + encodeURIComponent(json);
      ...
      $scope.yamlDownload = 'data:text/x-yaml;charset=utf-8,' + encodeURIComponent(yaml);
      if (jsonParseError) {
        assign(yaml, json);
      } else {
